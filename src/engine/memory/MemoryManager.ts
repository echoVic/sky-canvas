...
export class WebGLMemoryPool {
  ...
  expand(additionalSize: number): void {
    ...
    // 尝试与现有的最后一个自由块合并
    const lastFreeBlock = this.freeBlocks.find(block => 
      block.offset + block.size === expansionBlock.offset);
...
export class WebGLMemoryManager {
   ...
   * 批量分配
   */
  allocateBatch(requests: Array<{ type: PoolType; size: number; alignment?: number }>): MemoryBlock[] {
   ...
   * 获取池统计信息
   */
  getPoolStats(type: PoolType): { allocated: number; used: number; fragmentation: number; blocks: number } | null {
    const pool = this.pools.get(type);
