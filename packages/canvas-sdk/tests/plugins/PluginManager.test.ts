...
describe('PluginManager', () => {
  ...
  describe('插件加载', () => {
    ...
    it('应该拒绝加载重复ID的插件', async () => {
      ...
      await expect(
        pluginManager.loadPlugin(manifest, { default: MockPlugin })
    ...
    it('应该正确处理加载错误', async () => {
      ...
      // 模拟模块加载错误
      await expect(
        pluginManager.loadPlugin(manifest, null)
      ).rejects.toThrow();
  ...
  describe('插件激活', () => {
    ...
    it('应该拒绝激活不存在的插件', async () => {
      await expect(
        pluginManager.activatePlugin('non-existent')
  ...
  describe('插件停用', () => {
    ...
    it('应该拒绝停用不存在的插件', async () => {
      await expect(
        pluginManager.deactivatePlugin('non-existent')
      ).rejects.toThrow('not found');
    });
