...
describe.skip('PermissionManager', () => {
  ...
  describe('权限检查', () => {
    ...
    it('应该在任一权限被拒绝时返回false', async () => {
      ...
      const permissions = [PluginPermission.CANVAS_MODIFY, PluginPermission.FILE_ACCESS]
  ...
  describe('权限撤销', () => {
    ...
    it('应该成功撤销单个权限', () => {
      ...
      expect(permissionManager.hasPermission('test-plugin', PluginPermission.CANVAS_MODIFY)).toBe(
        false
      )
  ...
  describe('权限级别检查', () => {
    ...
    it('应该为高风险权限要求确认', async () => {
      ...
      expect(confirmSpy).toHaveBeenCalledWith(expect.stringContaining('SYSTEM_ACCESS'))
    })
  ...
  describe('事件系统', () => {
    it('应该触发权限授予事件', () => {
      ...
      expect(grantedSpy).toHaveBeenCalledWith('test-plugin', PluginPermission.CANVAS_MODIFY)
    })
    ...
    it('应该触发权限撤销事件', () => {
      ...
      expect(revokedSpy).toHaveBeenCalledWith('test-plugin', PluginPermission.CANVAS_MODIFY)
    })
  ...
  describe('权限导入导出', () => {
    ...
    it('应该导入权限配置', () => {
      ...
      expect(permissionManager.hasPermission('new-plugin', PluginPermission.NETWORK_ACCESS)).toBe(
        true
      )
      expect(permissionManager.hasPermission('new-plugin', PluginPermission.SYSTEM_ACCESS)).toBe(
        true
      )
    })
