...
describe.skip('PluginContext', () => {
  ...
  beforeEach(() => {
    ...
    pluginContext = new PluginContextImpl(mockManifest as any, mockPluginManager as any)
  })
  ...
  describe('画布API', () => {
    ...
    it('应该拒绝无权限的画布操作', async () => {
      ...
      await expect(pluginContext.api.canvas.addElement(element)).rejects.toThrow(
        'Permission denied'
      )
    })
  ...
  describe('文件系统API', () => {
    ...
    it('应该拒绝无权限的文件操作', async () => {
      ...
      await expect(pluginContext.api.fileSystem.readFile('test.txt')).rejects.toThrow(
        'Permission denied'
      )
    })
  })
