...
describe.skip('PerformanceMonitor', () => {
  ...
  describe('性能阈值检查', () => {
    it('应该检测加载时间超标', () => {
      ...
      expect(warnings).toContain(expect.stringContaining('slow-plugin'))
      expect(warnings).toContain(expect.stringContaining('load time'))
    })
    ...
    it('应该检测激活时间超标', () => {
      ...
      const warnings = performanceMonitor.getPerformanceWarnings()
      expect(warnings).toContain(expect.stringContaining('slow-plugin'))
      expect(warnings).toContain(expect.stringContaining('activation time'))
    })
    ...
    it('应该检测高错误率', () => {
      ...
      const warnings = performanceMonitor.getPerformanceWarnings()
      expect(warnings).toContain(expect.stringContaining('error-plugin'))
      expect(warnings).toContain(expect.stringContaining('error rate'))
    })
  })
