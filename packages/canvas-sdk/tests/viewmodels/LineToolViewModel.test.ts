...
describe('LineToolViewModel', () => {
  ...
  describe('initialization', () => {
    it('should initialize with correct default state', () => {
      ...
      expect(viewModel.state.endPoint).toBeNull();
  ...
  describe('drawing flow', () => {
    ...
    it('should create correct initial path data', () => {
      viewModel.handleMouseDown(50, 75);
      expect(viewModel.state.currentShape?.pathData).toBe('M 50 75 L 50 75');
    });

    it('should update end point and path on mousemove', () => {
      viewModel.handleMouseDown(100, 100);
      viewModel.handleMouseMove(200, 150);

      expect(viewModel.state.endPoint).toEqual({ x: 200, y: 150 });
      expect(viewModel.state.currentShape?.pathData).toBe('M 100 100 L 200 150');
    });

    it('should add shape on mouseup with valid length', () => {
      viewModel.handleMouseDown(100, 100);
    ...
    it('should not update on mousemove if not drawing', () => {
      viewModel.handleMouseMove(200, 200);
      expect(viewModel.state.endPoint).toBeNull();
    });
  });
  ...
  describe('helper methods', () => {
    ...
    it('getCurrentShape should return current shape', () => {
      ...
      expect(viewModel.getCurrentShape()?.type).toBe('path');
    });
