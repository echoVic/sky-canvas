...
import { getSingletonServiceDescriptors, type ServiceIdentifier } from './di/instantiation'
...
import {
  EllipseToolViewModel,
  IEllipseToolViewModel,
} from './viewmodels/tools/EllipseToolViewModel'
import { EraserToolViewModel, IEraserToolViewModel } from './viewmodels/tools/EraserToolViewModel'
import {
  EyedropperToolViewModel,
  IEyedropperToolViewModel,
} from './viewmodels/tools/EyedropperToolViewModel'
import { HandToolViewModel, IHandToolViewModel } from './viewmodels/tools/HandToolViewModel'
import { IImageToolViewModel, ImageToolViewModel } from './viewmodels/tools/ImageToolViewModel'
import { ILineToolViewModel, LineToolViewModel } from './viewmodels/tools/LineToolViewModel'
import {
  IPolygonToolViewModel,
  PolygonToolViewModel,
} from './viewmodels/tools/PolygonToolViewModel'
import {
  IRectangleToolViewModel,
  RectangleToolViewModel,
} from './viewmodels/tools/RectangleToolViewModel'
import { ISelectToolViewModel, SelectToolViewModel } from './viewmodels/tools/SelectToolViewModel'
import { IStarToolViewModel, StarToolViewModel } from './viewmodels/tools/StarToolViewModel'
...
class CanvasSDKBootstrap {
  ...
  private async initServices(): Promise<void> {
    ...
    // 获取核心服务
    this.logger = this.instantiationService.invokeFunction((accessor) => accessor.get(ILogService))
  ...
  private async startCore(): Promise<void> {
    ...
    const sceneManager = this.instantiationService.invokeFunction((accessor) =>
  ...
  getService<T>(serviceIdentifier: ServiceIdentifier<T>): T {
    ...
    return this.instantiationService.invokeFunction((accessor) => accessor.get(serviceIdentifier))
  }
  ...
  private registerCoreServices(services: ServiceCollection): void {
    ...
    services.set(IEllipseToolViewModel, new SyncDescriptor(EllipseToolViewModel))
    services.set(ILineToolViewModel, new SyncDescriptor(LineToolViewModel))
    services.set(IPolygonToolViewModel, new SyncDescriptor(PolygonToolViewModel))
    services.set(IStarToolViewModel, new SyncDescriptor(StarToolViewModel))
    ...
    services.set(IImageToolViewModel, new SyncDescriptor(ImageToolViewModel))
    services.set(IHandToolViewModel, new SyncDescriptor(HandToolViewModel))
    services.set(IEraserToolViewModel, new SyncDescriptor(EraserToolViewModel))
    services.set(IEyedropperToolViewModel, new SyncDescriptor(EyedropperToolViewModel))
