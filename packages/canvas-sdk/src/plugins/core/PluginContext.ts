...
export class PluginContextImpl implements PluginContext {
  ...
  dispose(): void {
    ...
    if (
      this.resources &&
      typeof (this.resources as PluginResourceManagerImpl).cleanup === 'function'
    ) {
    ...
    if (
      this.events &&
      typeof (this.events as PluginEventEmitterImpl).removeAllListeners === 'function'
    ) {
...
class PluginAPIImpl implements PluginAPI {
  ...
  canvas = {
    ...
    addShape: (shape: any) => {
      ...
      const event = new CustomEvent('plugin:addShape', {
        detail: { shape, pluginId: this.pluginId },
      })
    ...
    removeShape: (id: string) => {
      ...
      // 从画布移除图形
      const event = new CustomEvent('plugin:removeShape', {
    ...
    updateShape: (id: string, updates: any) => {
      ...
      const event = new CustomEvent('plugin:updateShape', {
        detail: { id, updates, pluginId: this.pluginId },
      })
      window.dispatchEvent(event)
    },
    ...
    addElement: (element: any) => {
      ...
      // 添加元素（兼容方法）
      const event = new CustomEvent('plugin:addShape', {
        detail: { shape: element, pluginId: this.pluginId },
      })
      window.dispatchEvent(event)
    },
  ...
  fileSystem = {
    readFile: async (path: string) => {
      ...
      // 读取文件
      const event = new CustomEvent('plugin:readFile', {
        detail: { path, pluginId: this.pluginId },
      })
      window.dispatchEvent(event)
      return ''
    },
    ...
    writeFile: async (path: string, content: string) => {
      ...
      // 写入文件
      const event = new CustomEvent('plugin:writeFile', {
        detail: { path, content, pluginId: this.pluginId },
      })
      window.dispatchEvent(event)
    },
    ...
    deleteFile: async (path: string) => {
      ...
      // 删除文件
      const event = new CustomEvent('plugin:deleteFile', {
        detail: { path, pluginId: this.pluginId },
      })
      window.dispatchEvent(event)
    },
    ...
    listFiles: async (path: string) => {
      ...
      // 列出文件
      const event = new CustomEvent('plugin:listFiles', {
        detail: { path, pluginId: this.pluginId },
      })
      window.dispatchEvent(event)
      return []
    ...
    createDirectory: async (path: string) => {
      ...
      // 创建目录
      const event = new CustomEvent('plugin:createDirectory', {
        detail: { path, pluginId: this.pluginId },
      })
      window.dispatchEvent(event)
    },
