...
export class ExtensionManager {
   ...
   * 注册扩展提供者
   */
  registerProvider(extensionPointId: string, provider: ExtensionProvider): void {
    ...
    if (providers.has(providerId)) {
      throw new Error(
        `Provider '${providerId}' already registered for extension point '${extensionPointId}'`
      )
    }
  ...
  unregisterProvider(extensionPointId: string, pluginId: string, extensionId: string): void {
    ...
    if (provider) {
      ...
      const index = extensionPoint.providers.findIndex(
        (p) => p.pluginId === pluginId && p.extensionId === extensionId
      )
      if (index >= 0) {
  ...
  getExtensionPointsByType(type: ExtensionPointType): ExtensionPoint[] {
    return Array.from(this.extensionPoints.values()).filter((ep) => ep.type === type)
  }
  ...
  getPluginProviders(pluginId: string): Map<string, ExtensionProvider[]> {
    ...
    for (const [extensionPointId, providers] of this.providers) {
      const pluginProviders = Array.from(providers.values()).filter(
