...
import type { ResourceInfo } from '../types/MemoryTypes'
...
export class ResourceTracker {
  ...
  registerResource(
    pluginId: string,
    resource: Omit<ResourceInfo, 'pluginId' | 'created' | 'lastAccessed'>
  ): string {
  ...
  getPluginResources(pluginId: string): ResourceInfo[] {
    return Array.from(this.resources.values()).filter((resource) => resource.pluginId === pluginId)
  }
