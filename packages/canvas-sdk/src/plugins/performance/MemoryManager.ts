...
export class MemoryManager {
  ...
  forceGarbageCollection(): GarbageCollectionResult {
    const resources = this.resourceTracker.getAllResources()
    return this.garbageCollector.forceGarbageCollection(resources, (resourceId: string) =>
      this.resourceTracker.releaseResource(resourceId)
    )
  }
  ...
  getOptimizationSuggestions(): string[] {
    ...
    for (const [pluginId, size] of systemStats.byPlugin) {
      if (size > 10 * 1024 * 1024) {
        // 10MB
        suggestions.push(
          `Plugin ${pluginId} is using ${Math.round(size / 1024 / 1024)}MB of memory`
        )
      }
    }
  ...
  getMemoryStats(): {
    ...
    return {
      ...
      averageMemoryPerPlugin:
        systemStats.byPlugin.size > 0 ? systemStats.totalSize / systemStats.byPlugin.size : 0,
      largestPlugin,
      largestPluginSize,
      plugins,
    }
  ...
  detectMemoryLeaks(): Array<{
    ...
    for (const [pluginId] of this.registeredResources) {
      ...
      if (analysis.isGrowing && analysis.growthRate > 1.0) {
        // 1MB/s
        const pluginResources = this.registeredResources.get(pluginId)
