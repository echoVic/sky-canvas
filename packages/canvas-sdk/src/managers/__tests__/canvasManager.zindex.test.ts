...
describe('CanvasManager Z轴管理', () => {
  ...
  beforeEach(() => {
    ...
    mockShapeService = {
      getShapeEntity: vi.fn((id: string) => testShapes.find((shape) => shape.id === id) || null),
  ...
  describe('bringToFront', () => {
    it('应该将指定的形状置顶', () => {
      ...
      expect(mockZIndexService.bringToFront).toHaveBeenCalledWith([testShapes[0]], testShapes)
    })
  ...
  describe('sendToBack', () => {
    it('应该将指定的形状置底', () => {
      ...
      expect(mockZIndexService.sendToBack).toHaveBeenCalledWith([testShapes[1]], testShapes)
    })
  })
  ...
  describe('sendBackward', () => {
    it('应该将指定的形状下移一层', () => {
      ...
      expect(mockZIndexService.sendBackward).toHaveBeenCalledWith([testShapes[2]], testShapes)
    })
  })
  ...
  describe('setZIndex', () => {
    it('应该设置指定形状的zIndex值', () => {
      ...
      expect(mockZIndexService.setZIndex).toHaveBeenCalledWith([testShapes[0]], targetZIndex)
    })

    it('应该批量更新形状实体', () => {
      const updatedShapes = [{ ...testShapes[0], zIndex: 5 }]
      ...
      expect(mockShapeService.updateShape).toHaveBeenCalledWith(testShapes[0].id, { zIndex: 5 })
    })
  })
  ...
  describe('批量更新', () => {
    it('batchUpdateShapes应该更新所有提供的形状', () => {
      ...
      expect(mockShapeService.updateShape).toHaveBeenCalledWith(testShapes[0].id, { zIndex: 10 })
      expect(mockShapeService.updateShape).toHaveBeenCalledWith(testShapes[1].id, { zIndex: 11 })
    })
  })
})
