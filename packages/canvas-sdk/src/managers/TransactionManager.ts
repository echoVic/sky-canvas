...
import type { ICommand, IHistoryService } from '../services/history/historyService'
...
export class TransactionManager implements ITransactionManager {
  ...
  getTransactionStats(): {
    ...
    const totalCommands = this.transactionStack.reduce(
      (sum, transaction) => sum + transaction.size(),
      0
    )
...
export function transactional(name?: string) {
  return (
    ...
    descriptor.value = function (...args: unknown[]) {
      const transactionManager: ITransactionManager | undefined = (
        this as { transactionManager?: ITransactionManager }
      ).transactionManager
