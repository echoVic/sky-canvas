...
export class ExportFileCommand extends AsyncCommand {
  ...
  constructor(model: CanvasModel, params: ExportFileParams) {
   ...
   * 检查形状是否在导出区域内
   */
  private isShapeInBounds(shape: any, bounds: { x: number; y: number; width: number; height: number }): boolean {
    const shapeRight = shape.x + (shape.width || shape.radius * 2 || 0);
    const shapeBottom = shape.y + (shape.height || shape.radius * 2 || 0);
   ...
   * 生成导出内容
   */
  private async generateContent(shapes: any[]): Promise<string> {
   ...
   * 生成JSON内容
   */
  private generateJsonContent(shapes: any[]): string {
    const data = {
      version: '1.0',
      timestamp: Date.now(),
   ...
   * 生成SVG内容
   */
  private generateSvgContent(shapes: any[]): string {
    // 计算SVG边界
...
`;
   ...
   * 生成图片内容（需要Canvas API）
   */
  private async generateImageContent(shapes: any[]): Promise<string> {
   ...
   * 计算形状边界
   */
  private calculateBounds(shapes: any[]): { x: number; y: number; width: number; height: number } {
    if (shapes.length === 0) {
    ...
    for (const shape of shapes) {
      const left = shape.x;
      const top = shape.y;
      const right = shape.x + (shape.width || shape.radius * 2 || 100);
      const bottom = shape.y + (shape.height || shape.radius * 2 || 100);
   ...
   * 将形状转换为SVG元素
   */
  private shapeToSvgElement(shape: any): string {
    ...
    switch (shape.type) {
      case 'rectangle':
        return `<rect x="${shape.x}" y="${shape.y}" width="${shape.width}" height="${shape.height}" ${commonAttrs} />\n`;
      ...
      case 'text':
        ...
        return `<text x="${shape.x}" y="${shape.y}" font-size="${fontSize}" ${commonAttrs}>${shape.text || ''}</text>\n`;
   ...
   * 在Canvas上绘制形状
   */
  private async drawShapeOnCanvas(ctx: CanvasRenderingContext2D, shape: any, bounds: { x: number; y: number }): Promise<void> {
    const style = shape.style || {};
    ...
    switch (shape.type) {
      case 'rectangle':
        if (style.fill) {
          ...
          ctx.fillRect(adjustedX, adjustedY, shape.width, shape.height);
        }
        if (style.stroke && style.strokeWidth) {
          ctx.strokeStyle = style.stroke;
          ctx.lineWidth = style.strokeWidth;
          ctx.strokeRect(adjustedX, adjustedY, shape.width, shape.height);
        }
        break;

      case 'circle':
        ctx.beginPath();
        ctx.arc(adjustedX, adjustedY, shape.radius, 0, 2 * Math.PI);
        if (style.fill) {
          ctx.fillStyle = style.fill;
          ctx.fill();
        }
      ...
      case 'text':
        if (shape.text) {
          ...
          if (style.fill) {
            ctx.fillStyle = style.fill;
            ctx.fillText(shape.text, adjustedX, adjustedY);
          }
          if (style.stroke && style.strokeWidth) {
            ctx.strokeStyle = style.stroke;
            ctx.lineWidth = style.strokeWidth;
            ctx.strokeText(shape.text, adjustedX, adjustedY);
          }
        }
        break;
    }
