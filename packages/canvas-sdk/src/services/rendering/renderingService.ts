 ...
 * 渲染服务标识符
 */
export const ICanvasRenderingService =
  createDecorator<ICanvasRenderingService>('CanvasRenderingService')
...
export class CanvasRenderingService implements ICanvasRenderingService {
  ...
  async initialize(canvas: HTMLCanvasElement, config: IRenderingConfig): Promise<void> {
    ...
    try {
      ...
      this.defaultLayer = engine.createLayer(
        CanvasRenderingService.DEFAULT_LAYER_ID,
        0
      ) as IRenderLayer
