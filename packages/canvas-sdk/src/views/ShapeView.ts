...
import { ShapeEntity, IRectangleEntity, ICircleEntity, IPathEntity, ITextEntity } from '../models/entities/Shape';
...
export class ShapeView {
   ...
   * 批量渲染形状
   */
  renderShapes(shapes: ShapeEntity[], context: IShapeRenderContext, selectedIds: Set<string> = new Set()): void {
    // 按 zIndex 排序
   ...
   * 应用形状样式
   */
  private applyShapeStyle(ctx: CanvasRenderingContext2D, shape: ShapeEntity, isSelected: boolean, isHovered: boolean): void {
   ...
   * 获取形状在屏幕上的边界框
   */
  getScreenBounds(shape: ShapeEntity, viewport: IViewportState): { x: number; y: number; width: number; height: number } {
    // TODO: 实现精确的屏幕边界计算
