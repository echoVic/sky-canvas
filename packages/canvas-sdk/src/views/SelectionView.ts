...
import type { ISelectionState, IViewportState } from '../viewmodels/interfaces/IViewModel'

/**
 * 获取形状边界的工具函数
 */
function getShapeBounds(shape: ShapeEntity): {
  x: number
  y: number
  width: number
  height: number
...
export class SelectionView {
   ...
   * 渲染多选择框
   */
  private renderMultipleSelection(
    ctx: CanvasRenderingContext2D,
   ...
   * 获取控制手柄位置
   */
  private getControlHandles(bounds: {
    x: number
    y: number
    width: number
    height: number
  ...
  hitTestHandle(
    ...
    const distToRotate = Math.sqrt(
      (worldPoint.x - rotateHandleX) ** 2 + (worldPoint.y - rotateHandleY) ** 2
    )
