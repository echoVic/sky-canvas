...
declare module './WebGPUContextImpl' {
  interface WebGPUContext {
    ...
    getStats(): { drawCalls: number; triangles: number; vertices: number; textureMemory: number; bufferMemory: number };
  }
}
...
Object.assign(WebGPUContext.prototype, {
  beginFrame(this: WebGPUContext): void {
    ...
    (this as unknown as { renderer: { updateTransform: (t: unknown) => void } }).renderer?.updateTransform(this.getState().transform);
  },
  ...
  fillRect(this: WebGPUContext, x: number, y: number, width: number, height: number): void {
    ...
    (this as unknown as { renderer: { fillRect: (x: number, y: number, w: number, h: number, c: Color) => void } }).renderer?.fillRect(x, y, width, height, color);
  },
  ...
  strokeRect(this: WebGPUContext, x: number, y: number, width: number, height: number): void {
    ...
    (this as unknown as { renderer: { strokeRect: (x: number, y: number, w: number, h: number, lw: number, c: Color) => void } }).renderer?.strokeRect(x, y, width, height, lineWidth, color);
  },

  fillCircle(this: WebGPUContext, x: number, y: number, radius: number): void {
    const state = this.getState();
    const color = toColor(state.style.fillColor as IColor, state.style.opacity);
    (this as unknown as { renderer: { fillCircle: (x: number, y: number, r: number, c: Color) => void } }).renderer?.fillCircle(x, y, radius, color);
  },
  ...
  drawLine(this: WebGPUContext, x1: number, y1: number, x2: number, y2: number): void {
    ...
    const color = toColor(state.style.strokeColor as IColor, state.style.opacity ?? 1);
    const lineWidth = state.style.lineWidth ?? 1;
    (this as unknown as { renderer: { drawLine: (x1: number, y1: number, x2: number, y2: number, lw: number, c: Color) => void } }).renderer?.drawLine(x1, y1, x2, y2, lineWidth, color);
  },

  // 路径方法 - 暂未实现
  beginPath(): void { /* TODO */ },
  closePath(): void { /* TODO */ },
  moveTo(_x: number, _y: number): void { /* TODO */ },
  lineTo(_x: number, _y: number): void { /* TODO */ },
  quadraticCurveTo(_cpx: number, _cpy: number, _x: number, _y: number): void { /* TODO */ },
  bezierCurveTo(_cp1x: number, _cp1y: number, _cp2x: number, _cp2y: number, _x: number, _y: number): void { /* TODO */ },
  arc(_x: number, _y: number, _radius: number, _startAngle: number, _endAngle: number, _counterclockwise?: boolean): void { /* TODO */ },
  rect(_x: number, _y: number, _width: number, _height: number): void { /* TODO */ },
  fill(): void { /* TODO */ },
  stroke(): void { /* TODO */ },
  ...
  // 裁剪方法
  clip(): void { /* TODO */ },
  clipRect(_x: number, _y: number, _width: number, _height: number): void { /* TODO */ },

  // 坐标转换
  screenToWorld(point: IPoint): IPoint { return { ...point }; },
  worldToScreen(point: IPoint): IPoint { return { ...point }; },

  // 其他设置方法
  setLineDash(_segments: number[]): void { /* TODO */ },
  setTextAlign(_align: 'left' | 'center' | 'right' | 'start' | 'end'): void { /* TODO */ },
  setTextBaseline(_baseline: 'top' | 'middle' | 'bottom' | 'alphabetic' | 'hanging'): void { /* TODO */ },
  setFont(_font: string): void { /* TODO */ },

  drawRect(this: WebGPUContext, rect: IPoint & { width: number; height: number }, fill = true, stroke = false): void {
    if (fill) this.fillRect(rect.x, rect.y, rect.width, rect.height);
    if (stroke) this.strokeRect(rect.x, rect.y, rect.width, rect.height);
  },

  drawCircle(this: WebGPUContext, center: IPoint, radius: number, fill = true, stroke = false): void {
  ...
  resize(this: WebGPUContext, width: number, height: number): void {
    ...
    (this as unknown as { renderer: { resize: (w: number, h: number) => void } }).renderer?.resize(width, height);
  },

  setViewport(_x: number, _y: number, _width: number, _height: number): void { /* TODO */ },
  getViewport(this: WebGPUContext): Rectangle {
    return (this as unknown as { renderState: { viewport: Rectangle } }).renderState.viewport;
  },
  setScissorTest(_enabled: boolean, _rect?: Rectangle): void { /* TODO */ },
  setBlendMode(_mode: string): void { /* TODO */ },
  setDepthTest(_enabled: boolean): void { /* TODO */ },
  setCullFace(_enabled: boolean): void { /* TODO */ },
  ...
  getStats(this: WebGPUContext): { drawCalls: number; triangles: number; vertices: number; textureMemory: number; bufferMemory: number } {
    const stats = (this as unknown as { renderer: { getStats: () => { drawCalls: number; triangles: number; vertices: number } } }).renderer?.getStats() ?? { drawCalls: 0, triangles: 0, vertices: 0 };
