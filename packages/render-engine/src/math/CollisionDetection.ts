...
import { type ISpatialObject, SpatialPartitionManager } from './SpatialPartitioning'
...
export class CollisionDetector<T extends ICollisionObject> {
  ...
  geometryCollision(geometryA: IGeometry, geometryB: IGeometry): IGeometryCollisionResult {
    ...
    if (!GeometryUtils.boundsIntersect(geometryA.bounds, geometryB.bounds)) {
      return {
        hasCollision: false,
        distance: new Vector2(geometryA.center.x, geometryA.center.y).distance(
          new Vector2(geometryB.center.x, geometryB.center.y)
        ),
        normal: new Vector2(0, 0),
        penetration: 0,
        contactPoint: geometryA.center,
    ...
    if (geometryA.type === GeometryType.CIRCLE && geometryB.type === GeometryType.CIRCLE) {
      return GeometryUtils.circleCircleCollision(
        geometryA as ICircleGeometry,
        geometryB as ICircleGeometry
      )
      ...
      const circle =
        geometryA.type === GeometryType.CIRCLE
          ? (geometryA as ICircleGeometry)
          : (geometryB as ICircleGeometry)
      const rect =
        geometryA.type === GeometryType.RECT
          ? (geometryA as IRectGeometry)
          : (geometryB as IRectGeometry)
  ...
  private pointIntersectsGeometry(point: IPoint, geometry: IGeometry): boolean {
    switch (geometry.type) {
      case GeometryType.CIRCLE: {
        ...
        const distance = new Vector2(point.x, point.y).distance(
          new Vector2(circleGeom.center.x, circleGeom.center.y)
        )
      ...
      default: {
        ...
        return (
          point.x >= defaultBounds.x &&
          point.x <= defaultBounds.x + defaultBounds.width &&
          point.y >= defaultBounds.y &&
          point.y <= defaultBounds.y + defaultBounds.height
        )
      }
    }
  }
  ...
  private raycastCircle(
    ...
    const halfChordLength = Math.sqrt(
      circle.radius * circle.radius - distanceToCenter * distanceToCenter
    )
  ...
  private raycastPolygon(
    ...
    direction: Vector2,
    maxDistance: number,
    polygon: IPolygonGeometry
  ): IRaycastResult {
    const rayStart = new Vector2(origin.x, origin.y)
    const rayDir = direction.normalize()

    let closestHit: IRaycastResult = {
      hit: false,
      point: origin,
      distance: Infinity,
      normal: new Vector2(0, 0),
