...
export class Matrix2D {
  ...
  static scale(scaleX: number, scaleY: number): Matrix2D {
    return new Matrix2D([scaleX, 0, 0, 0, scaleY, 0, 0, 0, 1])
  }
  ...
  getScale(): { x: number; y: number } {
    const scaleX = Math.sqrt(
      this.elements[0] * this.elements[0] + this.elements[3] * this.elements[3]
    )
    const scaleY = Math.sqrt(
      this.elements[1] * this.elements[1] + this.elements[4] * this.elements[4]
    )
