...
import { type AtlasEntry, TextureAtlas, type TextureInfo } from './TextureAtlas'
...
export class TextureLoader extends EventEmitter<LoaderEvents> {
  ...
  async loadTexture(
    url: string,
    options: LoadOptions = {}
  ): Promise<AtlasEntry | HTMLImageElement> {
  ...
  getProgress(): { completed: number; total: number; percentage: number } {
    ...
    const completed = Array.from(this.tasks.values()).filter(
      (task) => task.state === TextureLoadState.LOADED
    ).length
  ...
  cleanup(): void {
    const completedTasks = Array.from(this.tasks.entries()).filter(
      ([, task]) => task.state === TextureLoadState.LOADED || task.state === TextureLoadState.ERROR
    )
   ...
   */
  private waitForTask(
    task: TextureLoadTask
  ): Promise<AtlasEntry | HTMLImageElement | HTMLCanvasElement | ImageData> {
    return new Promise((resolve, reject) => {
