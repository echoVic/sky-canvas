...
export class TextureAtlas extends EventEmitter<AtlasEvents> {
  ...
  addTextures(textures: TextureInfo[]): Map<string, AtlasEntry> {
    ...
    // 按面积排序，大纹理优先
    const sortedTextures = [...textures].sort((a, b) => b.width * b.height - a.width * a.height)
  ...
  private drawTextureToAtlas(texture: TextureInfo, entry: AtlasEntry): void {
    ...
    try {
      atlas.context.drawImage(texture.data as any, entry.x, entry.y, entry.width, entry.height)
    } catch (error) {
  ...
  private handleMemoryPressure(): void {
    ...
    // 移除最久未使用的纹理
    const entries = Array.from(this.textureMap.values()).sort(
