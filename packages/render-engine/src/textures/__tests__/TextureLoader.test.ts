...
import { LoadOptions, TextureLoader, TextureLoadState } from '../TextureLoader'
...
describe('TextureLoader', () => {
  ...
  describe('优先级管理', () => {
    it('应该按优先级顺序加载', async () => {
      ...
      const promises = urls.map(({ url, priority }) => loader.loadTexture(url, { priority }))
  ...
  describe('缓存和重复加载', () => {
    ...
    it('应该避免重复加载相同URL', async () => {
      ...
      const promises = [loader.loadTexture(url), loader.loadTexture(url), loader.loadTexture(url)]
  ...
  describe('进度监控', () => {
    it('应该跟踪加载进度', async () => {
      const urls = ['https://example.com/progress1.png', 'https://example.com/progress2.png']
  ...
  describe('事件系统', () => {
    it('应该发出任务开始事件', async () => {
      ...
      expect([
        TextureLoadState.PENDING,
        TextureLoadState.LOADING,
        TextureLoadState.LOADED,
  ...
  describe('并发控制', () => {
    it('应该限制并发加载数量', async () => {
      const urls = Array.from({ length: 20 }, (_, i) => `https://example.com/concurrent${i}.png`)
