...
export class UnifiedPerformanceMonitor extends EventEmitter<UnifiedPerformanceEvents> {
  ...
  /** 获取指标统计 */
  getStats(
    metricType?: UnifiedMetricType
  ): UnifiedMetricStats | Map<UnifiedMetricType, UnifiedMetricStats> {
    if (metricType) {
  ...
  generateComprehensiveReport(): {
    ...
    return {
      ...
      bottlenecks,
      correlations,
      recommendations: this.suggestionProvider.generateRecommendations(
        bottlenecks,
        correlations,
        highWarningsCount
      ),
