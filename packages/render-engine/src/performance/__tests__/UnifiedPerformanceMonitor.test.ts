...
describe('UnifiedPerformanceMonitor', () => {
  ...
  beforeEach(() => {
    ...
    renderEngineAdapter = new MockDataSourceAdapter(DataSourceType.RENDER_ENGINE, [
      UnifiedMetricType.FPS,
      UnifiedMetricType.DRAW_CALLS,
      UnifiedMetricType.GPU_MEMORY,
    ])

    canvasSDKAdapter = new MockDataSourceAdapter(DataSourceType.CANVAS_SDK, [
  ...
  describe('数据源适配器', () => {
    ...
    it('应该正确处理适配器错误', async () => {
      // 创建会抛出错误的适配器
      const errorAdapter = new MockDataSourceAdapter(DataSourceType.FRONTEND_UI, [
  ...
  describe('警告系统', () => {
    ...
    it('应该触发低FPS警告', () => {
      ...
      const lowFpsWarning = newWarnings.find(
        (w) => w.type === UnifiedMetricType.FPS && w.severity === 'high'
      )
    ...
    it('应该触发高内存使用警告', () => {
      ...
      const memoryWarning = newWarnings.find((w) => w.type === UnifiedMetricType.MEMORY_USAGE)
