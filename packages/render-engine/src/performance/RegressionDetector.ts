...
export class RegressionDetector {
  ...
  getRecommendedSampleSize(
    ...
    const n = ((zAlpha + zBeta) / effectSize) ** 2 * 2
  ...
  private calculateStatistics(results: BenchmarkResult[]): StatisticalSummary {
    ...
    const variance = scores.reduce((sum, val) => sum + (val - mean) ** 2, 0) / (scores.length - 1)
  ...
  private performTTest(sample1: number[], sample2: number[]): TTestResult | null {
    ...
    const var1 = sample1.reduce((sum, val) => sum + (val - mean1) ** 2, 0) / (n1 - 1)
    const var2 = sample2.reduce((sum, val) => sum + (val - mean2) ** 2, 0) / (n2 - 1)
  ...
  private gamma(z: number): number {
    ...
    return Math.sqrt(2 * Math.PI) * t ** (z + 0.5) * Math.exp(-t) * result
  }
...
export class PerformanceAlertSystem {
  ...
  private calculateSeverity(
    regressions: Array<{ magnitude: number; confidence: number }>
