...
export class FrontendUIAdapter implements IDataSourceAdapter {
  ...
  private async collectBrowserMetrics(): Promise<Map<UnifiedMetricType, number>> {
    ...
    try {
      ...
      // 合并内存使用
      const currentMemory = metrics.get(UnifiedMetricType.MEMORY_USAGE) || 0;
      this.mapMetric(metrics, UnifiedMetricType.MEMORY_USAGE, currentMemory + browserMetrics.memoryUsage);
  ...
  private async collectWebPerformanceMetrics(): Promise<Map<UnifiedMetricType, number>> {
    ...
    try {
      ...
      if ('performance' in window) {
        ...
        const layoutMeasures = measureEntries.filter(entry => 
          entry.name.includes('layout') || entry.name.includes('reflow')
        );
        
        if (layoutMeasures.length > 0) {
          const avgLayoutTime = layoutMeasures.reduce((sum, entry) => sum + entry.duration, 0) / layoutMeasures.length;
        ...
        if (this.renderTimeCache.length > 0) {
          const avgRenderTime = this.renderTimeCache.reduce((sum, time) => sum + time, 0) / this.renderTimeCache.length;
...
export class FrontendUIPerformanceHelper {
  ...
  updateUIMetrics(): void {
    if (typeof document !== 'undefined') {
      ...
      this.uiMetrics.cssRuleCount = Array.from(document.styleSheets)
        .reduce((total, sheet) => {
          try {
