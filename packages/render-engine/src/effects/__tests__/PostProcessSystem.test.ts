...
describe('后处理效果系统', () => {
  ...
  describe('颜色调整效果', () => {
    ...
    it('对比度效果应该调整像素对比度', () => {
      ...
      imageData.data.set([100, 100, 100, 255, 200, 200, 200, 255])
  ...
  describe('工具函数', () => {
    ...
    it('应该提供效果分类', () => {
      expect(getPostProcessEffectCategory(PostProcessType.BRIGHTNESS)).toBe(
        PostProcessCategory.COLOR_ADJUSTMENT
      )
      expect(getPostProcessEffectCategory(PostProcessType.SHARPEN)).toBe(
        PostProcessCategory.IMAGE_ENHANCEMENT
      )
      expect(getPostProcessEffectCategory(PostProcessType.BLOOM)).toBe(
        PostProcessCategory.SPECIAL_EFFECTS
      )
