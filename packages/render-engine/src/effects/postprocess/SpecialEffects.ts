...
export class HDRTonemapEffect extends BasePostProcessEffect {
  ...
  apply(imageData: ImageData, targetData?: ImageData): ImageData {
    ...
    const exposureMultiplier = Math.pow(2, exposure * intensity);
...
export class DepthOfFieldEffect extends BasePostProcessEffect {
  ...
  apply(imageData: ImageData, targetData?: ImageData): ImageData {
    ...
    const data = imageData.data;
    const resultData = result.data;
    const { focusDistance, focalLength, fstop, maxBlur } = (this._config as DepthOfFieldConfig).parameters;
    const intensity = this._config.intensity;
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        resultData[index + 1] = data[index + 1] * invBlur + blurredImage.data[index + 1] * blurFactor;
        resultData[index + 2] = data[index + 2] * invBlur + blurredImage.data[index + 2] * blurFactor;
