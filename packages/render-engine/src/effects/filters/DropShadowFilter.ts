...
import { type DropShadowParameters, type FilterContext, FilterType } from '../types/FilterTypes'
...
export class DropShadowFilter extends BaseFilter<DropShadowParameters> {
  ...
  protected async processFilter(
    ...
    // 第三步：合成原图到阴影上方
    this.compositeImage(sourceImageData, blurredShadow, originalOffsetX, originalOffsetY)
    ...
    for (let y = 0; y < sourceImageData.height; y++) {
      for (let x = 0; x < sourceImageData.width; x++) {
        ...
        if (sourceAlpha > 0) {
          ...
          if (
            targetX >= 0 &&
            targetX < targetImageData.width &&
            targetY >= 0 &&
            targetY < targetImageData.height
          ) {
