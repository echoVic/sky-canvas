...
import { type FilterContext, FilterType, type InnerShadowParameters } from '../types/FilterTypes'
...
export class InnerShadowFilter extends BaseFilter<InnerShadowParameters> {
   ...
   * 创建边缘蒙版用于内阴影
   */
  private createEdgeMask(sourceImageData: ImageData, offsetX: number, offsetY: number): ImageData {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (currentAlpha > 0) {
          ...
          let offsetAlpha = 0
          if (offsetPosX >= 0 && offsetPosX < width && offsetPosY >= 0 && offsetPosY < height) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        if (maskAlpha > 0 && targetAlpha > 0) {
          ...
          targetData[index] = Math.round(
            targetData[index] * (1 - shadowStrength) + color.r * 255 * shadowStrength * 0.5
          )
          targetData[index + 1] = Math.round(
            targetData[index + 1] * (1 - shadowStrength) + color.g * 255 * shadowStrength * 0.5
          )
          targetData[index + 2] = Math.round(
            targetData[index + 2] * (1 - shadowStrength) + color.b * 255 * shadowStrength * 0.5
          )
        }
      }
    }
  ...
  protected validateSpecificParameters(parameters: InnerShadowParameters): boolean {
    if (
      typeof parameters.offsetX !== 'number' ||
