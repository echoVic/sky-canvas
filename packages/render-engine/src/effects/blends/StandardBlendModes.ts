...
export class NormalBlend extends BaseBlendOperation {
  ...
  apply(_baseColor: BlendColor, overlayColor: BlendColor): BlendColor {
    return {
      r: overlayColor.r,
      g: overlayColor.g,
      b: overlayColor.b,
...
export class PinLightBlend extends BaseBlendOperation {
  ...
  apply(baseColor: BlendColor, overlayColor: BlendColor): BlendColor {
    ...
    const r =
      overlay.r < 0.5 ? Math.min(base.r, 2 * overlay.r) : Math.max(base.r, 2 * (overlay.r - 0.5))
    const g =
      overlay.g < 0.5 ? Math.min(base.g, 2 * overlay.g) : Math.max(base.g, 2 * (overlay.g - 0.5))
    const b =
      overlay.b < 0.5 ? Math.min(base.b, 2 * overlay.b) : Math.max(base.b, 2 * (overlay.b - 0.5))
