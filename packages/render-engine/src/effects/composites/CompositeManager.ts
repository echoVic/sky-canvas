...
export class CompositeManager extends EventEmitter<CompositeEvents> implements ICompositeManager {
  ...
  private calculateBounds(layers: CompositeLayer[]): { x: number; y: number; width: number; height: number } {
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
  ...
  private updateStats(renderTime: number): void {
    ...
    this.stats.averageCompositeTime = 
      (this.stats.averageCompositeTime * (this.stats.totalComposites - 1) + renderTime) / this.stats.totalComposites;
