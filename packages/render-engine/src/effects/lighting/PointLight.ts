...
export class PointLight extends BaseLight {
  ...
  calculateLighting(
    ...
    const attenuation = this.calculateAttenuation(
      distance,
      this._config.radius,
      this._config.falloff
    )
    ...
    result.ambient = this.applyLightColor({ r: 255, g: 255, b: 255 }, ambientIntensity)

    // 漫反射分量
    const diffuseIntensity =
      this.calculateLambert(
    ...
    if (diffuseIntensity > 0) {
      result.diffuse = this.applyLightColor({ r: 255, g: 255, b: 255 }, diffuseIntensity)
    }

    // 镜面反射分量
    const specularIntensity =
      this.calculateBlinnPhong(
        { x: -normalizedLightDirection.x, y: -normalizedLightDirection.y },
        normal,
        viewDirection,
        material
    ...
    if (specularIntensity > 0) {
      result.specular = this.applyLightColor({ r: 255, g: 255, b: 255 }, specularIntensity)
    }
