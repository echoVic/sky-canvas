...
export abstract class BaseShadow implements IShadow {
  ...
  abstract render(ctx: CanvasRenderingContext2D, target: HTMLCanvasElement | ImageData): void
   ...
   * 创建模糊效果
   */
  protected applyBlur(ctx: CanvasRenderingContext2D, blurRadius: number): void {
    if (blurRadius > 0) {
   ...
   * 高斯模糊实现
   */
  protected gaussianBlur(imageData: ImageData, radius: number): ImageData {
    if (radius === 0) {
      return imageData
    }
