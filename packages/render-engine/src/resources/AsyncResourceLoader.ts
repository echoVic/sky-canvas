...
export class AsyncResourceLoader extends EventEmitter {
  ...
  private createTask(config: ResourceConfig): LoadingTask {
    ...
    const promise = new Promise<any>((resolve, reject) => {
      ...
      this.executeLoad(config, controller.signal, progress).then(resolve).catch(reject)
    })
  ...
  private async executeLoad(
    ...
    while (retries >= 0) {
      try {
        ...
        // 重试延迟
        await new Promise((resolve) => setTimeout(resolve, 2 ** (config.retries! - retries) * 1000))
      }
    }
