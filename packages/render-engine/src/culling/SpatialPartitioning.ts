...
export class CullingManager {
  ...
  constructor(
    spatialIndex: ISpatialNode | SpatialHashGrid,
...
export class CullingManagerFactory {
  ...
  static createOptimal(
    ...
    if (density > 0.01) {
      ...
      return this.createSpatialHash(Math.max(32, Math.min(256, cellSize)), cullMargin);
    } else {
      // 低密度场景，使用四叉树
      return this.createQuadTree(bounds, 15, 6, cullMargin);
    }
  }
}