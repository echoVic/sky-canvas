...
export class PathSimplification {
  ...
  private fitCubicBezier(
    ...
    // 尝试不同的结束点
    for (let endIndex = startIndex + 2; endIndex < Math.min(points.length, startIndex + 10); endIndex++) {
  ...
  editPath(path: Path, operation: PathEditOperation): Path {
    ...
    switch (operation.type) {
      case 'insert':
        if (operation.point && operation.segmentIndex >= 0 && operation.segmentIndex < newSegments.length) {
          // 在指定位置插入新的线段
