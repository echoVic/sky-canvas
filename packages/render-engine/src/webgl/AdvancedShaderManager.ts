...
class ShaderPreprocessor {
  ...
  static process(source: string, defines: Record<string, string | number | boolean> = {}): string {
    ...
    // 处理#include指令（简化实现）
    processed = ShaderPreprocessor.processIncludes(processed)

    // 处理条件编译
    processed = ShaderPreprocessor.processConditionalCompilation(processed, defines)
...
export class AdvancedShaderManager extends EventEmitter<ShaderManagerEvents> {
  ...
  private createShader(type: ShaderType, source: string): WebGLShader {
    ...
    if (!shader) {
      throw new Error(
        `Failed to create ${type === ShaderType.VERTEX ? 'vertex' : 'fragment'} shader`
      )
    }
