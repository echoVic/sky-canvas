 ...
 * 渲染批次
 */
export interface IRenderCommandBatch {
  /** 批次唯一标识 */
  readonly id: string;
 ...
 * 渲染批次实现
 */
export class RenderCommandBatch implements IRenderCommandBatch {
  ...
  constructor(materialKey: MaterialKey, commandType: RenderCommandType) {
    this.id = `batch_${++RenderCommandBatch.batchCounter}`;
  ...
  private materialsMatch(other: MaterialKey): boolean {
    ...
    return my.textureId === other.textureId &&
...
export class RenderQueue implements IRenderQueue {
  ...
  private batches: Map<string, IRenderCommandBatch> = new Map();
  ...
  private addToBatch(command: IRenderCommand): void {
    ...
    if (!batch) {
      batch = new RenderCommandBatch(command.materialKey, command.type);
    ...
    if (!batch.addCommand(command)) {
      ...
      const newBatch = new RenderCommandBatch(command.materialKey, command.type);
