...
import { BatchBuffer, BatchDataUtils, BatchKey, RenderBatch, Vertex } from '../core/BatchData';
import { BatchStats, IRenderable } from '../core/IBatchRenderer';
import { BatchContext, BatchData, IBatchStrategy } from '../core/IBatchStrategy';
...
export class InstancedStrategy implements IBatchStrategy {
   ...
   * 渲染实例化批次
   */
  private renderInstancedBatch(batch: InstancedBatch, projectionMatrix: Matrix3, context: BatchContext): void {
   ...
   * 渲染普通批次（非实例化）
   */
  private renderNormalBatch(batch: InstancedBatch, projectionMatrix: Matrix3, context: BatchContext): void {
    for (const instance of batch.instances) {
      const transformedVertices = this.transformVertices(batch.baseVertices, instance);
   ...
   * 渲染单个批次
   */
  private renderSingleBatch(batch: RenderBatch, projectionMatrix: Matrix3, context: BatchContext): void {
