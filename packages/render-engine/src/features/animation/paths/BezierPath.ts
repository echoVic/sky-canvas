...
export class QuadraticBezierPath extends BasePath {
  ...
  getPoint(t: number): Point2D {
    ...
    return {
      x: mt * mt * this.start.x + 2 * mt * clamped * this.control.x + clamped * clamped * this.end.x,
      y: mt * mt * this.start.y + 2 * mt * clamped * this.control.y + clamped * clamped * this.end.y
    };
  }
...
export class CubicBezierPath extends BasePath {
  ...
  getPoint(t: number): Point2D {
    ...
    return {
      x: mt3 * this.start.x + 3 * mt2 * clamped * this.control1.x + 
         3 * mt * t2 * this.control2.x + t3 * this.end.x,
      y: mt3 * this.start.y + 3 * mt2 * clamped * this.control1.y + 
         3 * mt * t2 * this.control2.y + t3 * this.end.y
    };
  }
