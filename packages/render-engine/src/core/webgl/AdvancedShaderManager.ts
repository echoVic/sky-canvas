...
import { ShaderType, ShaderSource, Shader } from './types';
...
export class AdvancedShaderManager extends EventEmitter3<ShaderManagerEvents> {
   ...
   * 获取着色器程序（支持变体）
   */
  getProgram(templateId: string, variantName?: string): Promise<ShaderProgram> {
  ...
  cleanupCache(force = false): number {
    ...
    for (const [key, cached] of this.compiledCache) {
      ...
      if (shouldRemove) {
        // cached.program.dispose(); // ShaderProgram接口暂不支持dispose方法
   ...
   * 同步编译着色器
   */
  private compileSync(template: ShaderTemplate, variant: ShaderVariant): ShaderProgram {
   ...
   * 异步编译着色器
   */
  private async compileAsync(template: ShaderTemplate, variant: ShaderVariant): Promise<ShaderProgram> {
   ...
   * 创建着色器程序
   */
  private createShaderProgram(vertexSource: string, fragmentSource: string, name: string): ShaderProgram {
   ...
   * 创建着色器程序包装器
   */
  private createShaderProgramWrapper(program: WebGLProgram, name: string): ShaderProgram {
    // 获取所有属性和uniform位置
    ...
    const fragmentShaderObj: Shader = {
      id: `${name}_fragment`,
   ...
   * 缓存编译后的程序
   */
  private cacheProgram(templateId: string, variant: ShaderVariant, program: ShaderProgram, compileTime: number): void {
    const cacheKey = `${templateId}_${variant.name}`;
