import { Shader, ShaderSource, ShaderType } from './types';
...
export class WebGLShaderManager implements IShaderManager {
  ...
  async loadShader(id: string, source: ShaderSource): Promise<boolean> {
    try {
      // 编译顶点着色器
      const vertexShader = new WebGLShader(this.gl, `${id}_vertex`, ShaderType.VERTEX, source.vertex);
      if (!await vertexShader.compile()) {
        return false;
      }

      // 编译片段着色器
      const fragmentShader = new WebGLShader(this.gl, `${id}_fragment`, ShaderType.FRAGMENT, source.fragment);
