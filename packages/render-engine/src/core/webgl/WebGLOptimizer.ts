...
import { IShaderManager, ShaderProgram } from './ShaderManager';
import { ShaderSource, ExtendedShaderSource } from './types';
...
export class ShaderCache {
   ...
   * 获取着色器程序（带缓存）
   */
  async getProgram(key: string, source: ShaderSource, shaderManager: IShaderManager): Promise<ShaderProgram | null> {
    ...
    // 编译新着色器
    const success = await shaderManager.loadShader(key, source);
    if (success) {
      const program = shaderManager.getShader(key);
      if (program) {
   ...
   * 预热着色器（异步编译）
   */
  warmupShader(key: string, source: ShaderSource, shaderManager: IShaderManager): void {
    if (this.compileQueue.has(key)) return;
    
    this.compileQueue.add(key);
    
    // 异步编译
    shaderManager.loadShader(key, source).then((success) => {
      if (success) {
        const program = shaderManager.getShader(key);
        if (program) {
          this.cache.set(key, program);
...
export class WebGLOptimizer extends EventEmitter3<WebGLOptimizerEvents> {
   ...
   * 获取优化的着色器程序
   */
  async getOptimizedShader(source: ShaderSource, shaderManager: IShaderManager): Promise<ShaderProgram | null> {
    // 生成着色器缓存键
    const key = `shader_${Math.random().toString(36).substr(2, 9)}`;
    const startTime = performance.now();
    
    const program = await this.shaderCache.getProgram(key, source, shaderManager);
    
    const compileTime = performance.now() - startTime;
    this.emit('shaderCompiled', { name: key, compileTime });
  ...
  private renderBatch(batch: OptimizedRenderBatch): void {
    ...
    for (const [name, value] of batch.uniforms.entries()) {
      const location = batch.shader.uniforms.get(name);
      if (location) {
        // 根据值类型设置uniform
   ...
   * 预热着色器
   */
  warmupShaders(sources: Array<{ key: string; source: ShaderSource }>, shaderManager: IShaderManager): void {
