...
import { OptimizationMetrics, createInitialMetrics } from './RenderOptimizationTypes';
import { RenderOptimizationConfig } from './RenderOptimizationConfig';
...
export class MetricsCollector {
  ...
  private updateMemoryMetrics(systems: Map<string, BaseSystem>, config: RenderOptimizationConfig): void {
  ...
  private updateRenderingMetrics(systems: Map<string, BaseSystem>): void {
    const pipelineScheduler = systems.get('pipeline-scheduler') as RenderPipelineScheduler | undefined;
  ...
  private updateGPUMetrics(systems: Map<string, BaseSystem>): void {
    ...
    if (gpuOptimizer) {
      ...
      this.metrics.gpu.utilization = gpuStats.availableMemory > 0
