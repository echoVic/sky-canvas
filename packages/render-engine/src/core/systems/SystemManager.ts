...
import { type ExtensionManager, ExtensionType } from './ExtensionSystem'
...
export class SystemManager {
    ...
    try {
      ...
      if (initErrors.length > 0) {
        ...
        if (criticalFailures.length > 0) {
          throw new Error(
            `Critical systems failed to initialize: ${criticalFailures.map((f) => f.systemName).join(', ')}`
          )
        }
      }
  ...
  private sortSystems(): void {
    this.sortedSystems = Array.from(this.systems.values()).sort((a, b) => b.priority - a.priority)
  }
}
