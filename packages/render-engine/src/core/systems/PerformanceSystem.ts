...
class FrustumCuller {
  ...
  cull(objects: ICullable[]): ICullable[] {
    ...
    return objects.filter((obj) => obj.visible && obj.isInViewport(expandedViewport, this.margin))
  }
}
...
export class PerformanceSystem extends BaseSystem {
  ...
  update(_deltaTime: number): void {
    ...
    this.stats.cacheHitRate =
      this.stats.cachedObjects / Math.max(1, this.stats.cachedObjects + this.stats.poolMisses)
