  ...
  private processConditionals(
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('#ifdef')) {
        ...
        const condition = Object.hasOwn(defines, macro)
...
export class ShaderOptimizationSystem extends BaseSystem {
   ...
   * 获取着色器程序
   */
  async getShaderProgram(
    name: string,
    variant: ShaderVariant = { defines: {}, features: [], hash: '' }
  ...
  private updateCompilationStats(compilationTime: number, success: boolean): void {
    if (success) {
      ...
      this.stats.averageCompilationTime =
        this.stats.totalCompilationTime / this.stats.compiledPrograms
    }
