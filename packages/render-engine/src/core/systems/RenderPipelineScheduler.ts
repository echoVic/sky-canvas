...
import type { BatchRenderSystem } from './BatchRenderSystem'
import type { GPUResourceOptimizer } from './GPUResourceOptimizer'
import type { PerformanceMonitorSystem } from './PerformanceMonitorSystem'
...
import {
  ...
  RenderPhase,
  type RenderStats,
  type TaskSchedulerStats,
} from './RenderPipelineTypes'
import type { ResourceSystem } from './ResourceSystem'
import type { ShaderOptimizationSystem } from './ShaderOptimizationSystem'
...
export class RenderPipelineScheduler extends BaseSystem {
  ...
  private triggerOptimizations(violations: string[]): void {
    for (const violation of violations) {
      switch (violation) {
        case 'frame_time':
          logger.debug('Optimizing frame time')
          break
        case 'draw_calls':
          logger.debug('Optimizing draw calls')
          break
        case 'triangles':
          logger.debug('Optimizing triangle count')
          break
        case 'memory_usage':
