...
// 重新导出预设配置
export { PRESET_CONFIGS, LOW_END_CONFIG, MEDIUM_END_CONFIG, HIGH_END_CONFIG } from './RenderOptimizationPresets';
...
export class RenderOptimizationConfigManager {
  ...
  validateConfig(config: RenderOptimizationConfig): boolean {
    try {
      ...
      const requiredKeys = [
        'lod', 'batching', 'culling', 'cache', 'gpuResource',
        'shader', 'monitoring', 'multiThread', 'adaptiveQuality', 'pipeline'
      ];
