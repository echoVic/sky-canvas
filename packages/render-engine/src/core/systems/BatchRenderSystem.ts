...
export class BatchRenderSystem extends BaseSystem {
  ...
  async init(): Promise<void> {
    ...
    if (
      renderContext &&
      'canvas' in renderContext &&
      renderContext.canvas instanceof HTMLCanvasElement
    ) {
  ...
  private flush(): void {
    ...
    this.gl.drawElements(this.gl.TRIANGLES, this.currentBatch.count * 6, this.gl.UNSIGNED_SHORT, 0)
  ...
  private uploadBatchData(): void {
    ...
    this.gl.bufferSubData(
      ...
      0,
      this.currentBatch.vertices.subarray(0, this.currentBatch.count * 4)
    )
    ...
    this.gl.bufferSubData(
      ...
      0,
      this.currentBatch.indices.subarray(0, this.currentBatch.count * 6)
    )
  }
