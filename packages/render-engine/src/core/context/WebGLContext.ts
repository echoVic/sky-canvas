...
export class WebGLContext implements IWebGLContext {
  ...
  setState(state: IGraphicsState): void {
    if (state.transform) {
      this.currentTransform = new Matrix3([
        state.transform.a, state.transform.b, 0,
        state.transform.c, state.transform.d, 0,
        state.transform.e, state.transform.f, 1
      ]);
    }
    if (state.style) {
  ...
  clip(): void {
    if (this.currentPath.length >= 3) {
      let minX = this.currentPath[0].x, minY = this.currentPath[0].y;
