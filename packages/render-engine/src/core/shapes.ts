...
import type { Drawable, RenderContext, RenderState } from './index'
...
export abstract class Shape implements Drawable {
  ...
  protected applyStyle(ctx: CanvasRenderingContext2D): void {
    ...
    if (this._style.globalCompositeOperation)
      ctx.globalCompositeOperation = this._style.globalCompositeOperation
...
export class Circle extends Shape {
  ...
  hitTest(point: IPoint): boolean {
    const localPoint = this.transform.inverseTransformPoint(new Vector2(point.x, point.y))
    const center = this.center
    const distance = Math.sqrt((localPoint.x - center.x) ** 2 + (localPoint.y - center.y) ** 2)
