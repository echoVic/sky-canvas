...
class StyleParser {
  ...
  static parseStyle(styleString: string): Partial<TextStyle> {
    ...
    for (const declaration of declarations) {
      ...
      StyleParser.parseStyleProperty(property, value, style)
    }
   ...
   * 解析单个CSS属性
   */
  private static parseStyleProperty(
    property: string,
    value: string,
    style: Partial<TextStyle>
  ): void {
    switch (property.toLowerCase()) {
      case 'color':
        style.color = StyleParser.parseColor(value)
        break
      ...
      case 'font-size':
        style.fontSize = StyleParser.parseSize(value)
        break

      case 'font-weight':
        style.fontWeight = StyleParser.parseFontWeight(value)
        break

      case 'font-style':
        style.fontStyle = StyleParser.parseFontStyle(value)
        break

      case 'line-height':
        style.lineHeight = StyleParser.parseLineHeight(value)
        break

      case 'text-decoration':
        style.textDecoration = StyleParser.parseTextDecoration(value)
        break

      case 'text-decoration-color':
        style.textDecorationColor = StyleParser.parseColor(value)
        break

      case 'text-align':
        style.textAlign = StyleParser.parseTextAlign(value)
        break

      case 'vertical-align':
        style.verticalAlign = StyleParser.parseVerticalAlign(value)
        break

      case 'text-transform':
        style.textTransform = StyleParser.parseTextTransform(value)
        break

      case 'letter-spacing':
        style.letterSpacing = StyleParser.parseSize(value)
        break

      case 'word-spacing':
        style.wordSpacing = StyleParser.parseSize(value)
        break
      ...
      case 'background-color':
        style.backgroundColor = StyleParser.parseColor(value)
        break

      case 'text-shadow':
        style.shadow = StyleParser.parseTextShadow(value)
        break
    }
  }
  ...
  private static parseColor(value: string): any {
    ...
    // 命名颜色
    return StyleParser.parseNamedColor(value)
  }
  ...
  private static parseTextShadow(value: string): any {
    ...
    const offsetX = StyleParser.parseSize(parts[0])
    const offsetY = StyleParser.parseSize(parts[1])
    const blurRadius = StyleParser.parseSize(parts[2])
    const color = parts[3] ? StyleParser.parseColor(parts[3]) : ColorUtils.fromRGB(0, 0, 0, 0.5)
...
class HTMLParser {
  ...
  static parseHTML(html: string): ParseNode[] {
    ...
    while (currentIndex < html.length) {
      ...
      if (tagString.startsWith('/')) {
        ...
        // 开始标签
        const elementNode = HTMLParser.parseTag(tagString)
        ...
        // 自闭合标签不需要入栈
        if (!HTMLParser.VOID_ELEMENTS.has(elementNode.tagName || '') && !tagString.endsWith('/')) {
  ...
  private static parseTag(tagString: string): ParseNode {
    ...
    const tagName =
      spaceIndex === -1 ? tagString.replace('/', '') : tagString.substring(0, spaceIndex)
    ...
    for (const node of nodes) {
      if (node.type === 'text') {
        ...
        if (node.children && node.children.length > 0) {
          index = this.processNodes(node.children, elementStyle, fragments, contentArray, index)
        }
      }
    }
