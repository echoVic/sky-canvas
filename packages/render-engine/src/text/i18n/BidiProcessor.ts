...
const BIDI_CHARACTER_MAP = new Map<number, CharacterClass>([
  // 拉丁字母 (A-Z, a-z)
  ...[...Array(26)].map((_, i) => [0x0041 + i, CharacterClass.LEFT_TO_RIGHT] as [number, CharacterClass]),
  ...[...Array(26)].map((_, i) => [0x0061 + i, CharacterClass.LEFT_TO_RIGHT] as [number, CharacterClass]),
  
  // 阿拉伯字母范围 (U+0600-U+06FF)
  ...[...Array(256)].map((_, i) => [0x0600 + i, CharacterClass.ARABIC_LETTER] as [number, CharacterClass]),
  
  // 希伯来字母 (U+05D0-U+05EA)
  ...[...Array(27)].map((_, i) => [0x05D0 + i, CharacterClass.RIGHT_TO_LEFT] as [number, CharacterClass]),
  
  // 欧洲数字 (0-9)
  ...[...Array(10)].map((_, i) => [0x0030 + i, CharacterClass.EUROPEAN_NUMBER] as [number, CharacterClass]),
  
  // 阿拉伯-印度数字 (U+0660-U+0669)
  ...[...Array(10)].map((_, i) => [0x0660 + i, CharacterClass.ARABIC_NUMBER] as [number, CharacterClass]),
...
export class BidiProcessor {
  ...
  private classifyByRange(codePoint: number): CharacterClass {
    ...
    if ((codePoint >= 0x0030 && codePoint <= 0x0039) || // 0-9
        (codePoint >= 0xFF10 && codePoint <= 0xFF19)) { // 全角数字
  ...
  detectBaseDirection(text: string): DirectionDetectionResult {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (charClass === CharacterClass.LEFT_TO_RIGHT) {
        ltrCount++;
      } else if (charClass === CharacterClass.RIGHT_TO_LEFT || 
   ...
   * 分析每个字符的双向类型
   */
  private analyzeCharacters(text: string): Array<{ char: string; class: CharacterClass; level: number }> {
   ...
   * 解析弱类型 (W1-W7)
   */
  private resolveWeakTypes(characters: Array<{ char: string; class: CharacterClass; level: number }>): void {
    // 这是简化版本，完整实现需要处理更多规则
  ...
  private resolveNeutralTypes(
    ...
    for (let i = 0; i < characters.length; i++) {
      ...
      if (char.class === CharacterClass.NEUTRAL || char.class === CharacterClass.OTHER_NEUTRAL) {
        // 简化处理：使用段落级别
        char.class = paragraphLevel === 0 ? CharacterClass.LEFT_TO_RIGHT : CharacterClass.RIGHT_TO_LEFT;
      }
    }
  }
  ...
  private resolveImplicitLevels(
    ...
    for (const char of characters) {
      ...
      } else if (char.class === CharacterClass.RIGHT_TO_LEFT || 
