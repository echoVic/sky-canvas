...
const LANGUAGE_PATTERNS = new Map<string, Map<string, number>>([
  ...
  ['fr', new Map([
    ['e', 17.26], ['s', 7.90], ['a', 7.64], ['r', 6.65], ['i', 7.24],
    ['t', 7.24], ['n', 7.15], ['u', 6.31], ['l', 5.46], ['o', 5.38]
  ])],
  
  ['de', new Map([
    ['e', 17.40], ['n', 9.78], ['s', 7.27], ['r', 7.00], ['i', 7.55],
    ['t', 6.15], ['u', 4.35], ['a', 6.51], ['d', 5.08], ['h', 4.76]
  ])],
  
  ['zh', new Map([
    // 中文使用字符频率
    ['的', 5.90], ['是', 2.44], ['在', 2.17], ['有', 1.67], ['我', 1.59],
    ['你', 1.45], ['了', 1.37], ['他', 1.25], ['这', 1.14], ['上', 1.02]
  ])],
]);
...
export class LanguageDetector {
  ...
  analyzeTextComplexity(text: string): TextAnalysisResult {
    ...
    const complexity = uniqueScripts > 1 || baseDirection === TextDirection.RTL ? 'complex' : 'simple';

    // 判断是否需要文本塑形
    const requiresShaping = detectedLanguage?.script === 'Arab' || 
