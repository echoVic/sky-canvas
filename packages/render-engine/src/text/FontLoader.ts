...
export class FontLoader extends EventEmitter<FontLoaderEvents> implements IFontLoader {
  ...
  private async performLoad(task: LoadingTask): Promise<ArrayBuffer> {
    ...
    try {
      ...
      if (!response.body) {
        throw new FontError('Response body is null', FontErrorCode.NETWORK_ERROR, undefined, source)
      }
...
export class FontLoadingUtils {
  ...
  static getBestFontFormat(sources: FontSource[]): FontSource | null {
    const formatPriority = [FontFormat.WOFF2, FontFormat.WOFF, FontFormat.TTF, FontFormat.OTF]
