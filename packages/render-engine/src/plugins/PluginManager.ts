...
import { deduplicatePackages, findDependentPackages, sortPackages } from './manager/PackageUtils'
import { isVersionCompatible, shouldUpdate } from './manager/VersionUtils'
import type { Plugin, PluginMetadata, PluginState, PluginSystem } from './PluginSystem'
...
export class PluginManager {
  ...
  async updatePlugin(packageId: string, options: Partial<UpdateOptions> = {}): Promise<void> {
    ...
    try {
      ...
      if (
        !opts.force &&
        !shouldUpdate(installedPackage.metadata.version, latestPackage.metadata.version)
      ) {
        return
      }
  ...
  async checkForUpdates(): Promise<
    Array<{ packageId: string; currentVersion: string; availableVersion: string }>
  > {
    const updates = []
