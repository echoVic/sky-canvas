...
import type { GPUParticleSystem, ParticleData } from './GPUParticleSystem'
...
export class ParticleEmitter {
  ...
  constructor(id: string, particleSystem: GPUParticleSystem, config: Partial<EmitterConfig> = {}) {
    this.id = id
  ...
  private updateContinuousEmission(deltaTime: number): void {
    ...
    while (
      ...
      this.totalEmittedParticles < this.config.maxParticles
    ) {
      this.emitParticle()
      this.emissionTimer -= emissionInterval
    }
  ...
  private generateParticleData(): Partial<ParticleData> {
    ...
    const color = {
      r: Math.max(
        0,
        Math.min(1, config.startColor.r + this.randomVariance(config.colorVariance.r))
      ),
      g: Math.max(
        0,
        Math.min(1, config.startColor.g + this.randomVariance(config.colorVariance.g))
      ),
      b: Math.max(
        0,
        Math.min(1, config.startColor.b + this.randomVariance(config.colorVariance.b))
      ),
      a: Math.max(
        0,
        Math.min(1, config.startColor.a + this.randomVariance(config.colorVariance.a))
      ),
    }
  ...
  private generateLinePosition(basePosition: { x: number; y: number; z: number }): {
    x: number
    y: number
    z: number
  } {
  ...
  private generateCirclePosition(basePosition: { x: number; y: number; z: number }): {
    x: number
    y: number
    z: number
  } {
  ...
  private generateRectanglePosition(basePosition: { x: number; y: number; z: number }): {
    x: number
    y: number
    z: number
  } {
  ...
  private generateConePosition(basePosition: { x: number; y: number; z: number }): {
    x: number
    y: number
    z: number
  } {
