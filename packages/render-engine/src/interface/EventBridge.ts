 ...
 * 事件过滤器接口
 */
export type EventFilter = (event: BridgeEvent) => boolean

/**
 * 事件转换器接口
 */
export type EventTransformer = (event: BridgeEvent) => BridgeEvent | null
...
export class EventBridge {
  ...
  addEventListener(type: BridgeEventType | '*', listener: BridgeEventListener): void {
    if (type === '*') {
      ...
      if (listeners.size >= this.config.maxListenersPerEvent) {
        console.warn(
          `Too many listeners for event ${type}, limit is ${this.config.maxListenersPerEvent}`
        )
        return
      }
  ...
  private async dispatchEvent(event: BridgeEvent): Promise<void> {
    ...
    try {
      ...
      if (processingTime > 16) {
        console.warn(
          `Slow event processing: ${event.type} took ${processingTime.toFixed(2)}ms with ${listenersNotified} listeners`
        )
      }
    } catch (error) {
  ...
  getStats() {
    return {
      ...
      listenerStats: {
        totalTypeListeners: Array.from(this.listeners.values()).reduce(
