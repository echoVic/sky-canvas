...
export class BridgeCommandOptimizer {
  ...
  private static areRectanglesAdjacent(rect1: any, rect2: any): boolean {
    ...
    // 水平相邻
    if (Math.abs(rect1.y - rect2.y) <= gap && 
    ...
    // 垂直相邻
    if (Math.abs(rect1.x - rect2.x) <= gap && 
  ...
  private setupInterceptors(): void {
    ...
    this.interceptor.addInterceptor('executeCommand', {
      ...
      after: async (context) => {
        ...
        if (duration > 16) { // 超过1帧时间
          console.warn(`Slow render command: ${context.args[0]?.type} took ${duration.toFixed(2)}ms`);
        }
        return context;
      }
    });
