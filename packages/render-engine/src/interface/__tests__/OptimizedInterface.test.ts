...
describe('BatchCallManager', () => {
  ...
  it('应该在达到最大批次时立即处理', () => {
    ...
    expect(processor).toHaveBeenCalledWith(
      expect.arrayContaining(['data0', 'data1', 'data2', 'data3', 'data4'])
    )
  })
...
describe('InterfaceInterceptor', () => {
  ...
  it('应该能够处理错误拦截器', async () => {
    ...
    await expect(interceptor.intercept('testMethod', originalFn, [])).rejects.toThrow(
      'Modified error'
    )
