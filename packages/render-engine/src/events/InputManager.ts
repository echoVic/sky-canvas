...
import { 
  ...
  BaseEvent, 
  ITouch,
  ITouchEvent as IInternalTouchEvent,
  IGestureEvent as IInternalGestureEvent
} from './InputEvents';
...
export class InputManager extends EventEmitter<InputManagerEvents> {
  ...
  private _convertToInternalTouchEvent(e: globalThis.TouchEvent, type: string): IInternalTouchEvent {
    const rect = this._element?.getBoundingClientRect();
