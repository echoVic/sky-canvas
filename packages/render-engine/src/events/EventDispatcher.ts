...
import type { EventListener, IBaseEvent } from './InputEvents'
...
export class EventDispatcher {
   ...
   * 移除事件监听器
   */
  removeEventListener<T extends IBaseEvent>(type: string, listener: EventListener<T>): void {
    if (this._disposed) {
      return
    }
   ...
   * 添加一次性事件监听器
   */
  once<T extends IBaseEvent>(type: string, listener: EventListener<T>): void {
