...
import { CubicBezierPath, QuadraticBezierPath } from '../paths/BezierPath'
...
describe('PathFactory', () => {
  ...
  describe('便捷创建方法', () => {
    it('应该创建直线', () => {
      const path = PathFactory.createLine({ x: 0, y: 0 }, { x: 100, y: 100 })
    ...
    it('应该创建圆形', () => {
      const path = PathFactory.createCircle({ x: 50, y: 50 }, 30, 0, Math.PI, false)
    ...
    it('应该创建椭圆', () => {
      const path = PathFactory.createEllipse({ x: 50, y: 50 }, 40, 30, Math.PI / 4)
  ...
  describe('几何形状创建', () => {
    ...
    it('应该创建星形路径', () => {
      const path = PathFactory.createStar({ x: 50, y: 50 }, 40, 20, 5, Math.PI / 4)
  ...
  describe('特殊曲线创建', () => {
    it('应该创建螺旋路径', () => {
      const path = PathFactory.createSpiral({ x: 50, y: 50 }, 10, 40, 3, true)
    ...
    it('应该创建波浪路径', () => {
      const path = PathFactory.createWave({ x: 0, y: 50 }, { x: 200, y: 50 }, 20, 3)
