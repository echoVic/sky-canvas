...
export class AnimationExamples {
  ...
  timelineSequence() {
    ...
    // 第一个元素立即开始
    timeline.at(0, this.manager.createMultiPropertyAnimation({
      target: elements[0],
      properties: {
        x: { to: 200 },
        opacity: { to: 1 }
    ...
    // 第二个元素延迟200ms开始
    timeline.at(200, this.manager.createMultiPropertyAnimation({
      target: elements[1],
      properties: {
        x: { to: 200 },
        opacity: { to: 1 }
    ...
    // 第三个元素再延迟200ms开始
    timeline.at(400, this.manager.createMultiPropertyAnimation({
      target: elements[2],
      properties: {
        x: { to: 200 },
        opacity: { to: 1 }
  ...
  complexTimelineChoreography() {
    ...
    // 标题下滑进入（与logo重叠300ms）
    timeline.at(700, this.manager.createMultiPropertyAnimation({
      target: title,
      properties: {
        y: { to: 0 },
        opacity: { to: 1 }
    ...
    // 按钮弹跳进入
    timeline.at(1200, this.manager.createMultiPropertyAnimation({
      target: button,
      properties: {
        scale: { to: 1 },
        opacity: { to: 1 }
  ...
  utilityFunctionExamples() {
    ...
    const fadeInMove = AnimationUtils.to(target, 1000, {
      x: 100,
      y: 50,
      opacity: 1
    }, {
    ...
    // 使用fromTo指定起始值
    const slideUp = AnimationUtils.fromTo(target, 800, 
      { y: 100 }, 
      { y: 0 }, 
      {
        easing: EasingType.EASE_OUT_BACK
