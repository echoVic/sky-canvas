...
import { IParticle, RotationAffectorConfig, ParticleRange } from '../../types/ParticleTypes';
...
export class RotationAffector extends BaseAffector {
  ...
  constructor(config: RotationAffectorConfig) {
    ...
    this.angularAccelerationRange = (config as RotationAffectorConfigExtended).angularAcceleration || { min: 0, max: 0 };
    this.curve = (config as RotationAffectorConfigExtended).curve ? [...(config as RotationAffectorConfigExtended).curve!] : null;
