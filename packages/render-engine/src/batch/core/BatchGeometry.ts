...
import type { BatchVertex, LineParams, QuadParams, TriangleParams } from '../types/BatchTypes'
import type { BatchBuffer } from './BatchBuffer'
  ...
  addCircle(
    ...
    for (let i = 0; i < segments; i++) {
      ...
      this.buffer.addVertex({
        ...
        color,
        texCoord: new Vector2(0.5 + Math.cos(angle) * 0.5, 0.5 + Math.sin(angle) * 0.5),
      })
    }
