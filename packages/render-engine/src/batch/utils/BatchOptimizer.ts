...
import { RenderBatch } from '../core/BatchData';
import { BatchStats } from '../core/IBatchRenderer';
...
export class BatchOptimizer {
  ...
  detectInstancingOpportunities(batches: RenderBatch[]): Array<{
    ...
    for (const [hash, group] of geometryGroups) {
      if (group.length >= 5) { // 至少5个相同几何体才值得实例化
  ...
  private calculateAverageStats(): BatchStats {
    ...
    const sum = this.performanceHistory.reduce((acc, stats) => ({
      ...
      drawCalls: 0, triangles: 0, vertices: 0, batches: 0,
      textureBinds: 0, shaderSwitches: 0, frameTime: 0
    });
  ...
  private generateSuggestions(analysis: {
    ...
    // 检查帧时间
    if (analysis.stats.frameTime > 16.67) { // 超过60fps的时间预算
      suggestions.push({
   ...
   * 比较趋势
   */
  private compareTrend(recent: number, older: number, higherIsBetter: boolean): 'improving' | 'stable' | 'degrading' {
